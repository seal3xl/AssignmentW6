...
# 若帳號未備註冊，完成註冊，記錄到user表，並在session表紀錄為logout。
        if usernames==():
            command_add="insert into user (name,username,passward) values(%s,%s,%s);"
            cursor.execute(command_add,(na,ac,ps))
            command_add="insert into session (username,state) values(%s,%s);"
            cursor.execute(command_add,(ac,"logout"))
            db.commit()
            return redirect("/")
...
# 若帳號密碼正確，將session表中的狀態改為login，並導向會員頁。
        else:
            session["ac"]=request.form["ac"]

            command_login="update session set state=\"login\" where username=\""+ac+"\";"
...
def signout():
    ac=session["ac"]
# 將session表中的狀態改為logout。
    cursor=db.cursor()
    command_logout="update session set state=\"logout\" where username=\""+ac+"\";"
    cursor.execute(command_logout)
    db.commit()
